...
def mergeDataDis_PTAP(ptap_id,user_id,study_case_id, scenario):
    ...
    for label,series in final.items():
        ...
        insertParameter(float(values[0]),ptap_id,values[1],user_id,values[2],values[3],values[4],values[5],values[6],values[7],values[8],values[9],values[10],0,study_case_id,scenario)

def insertParameter(element_id,ptap_id,year,user_id,awy,cn_mg_l, cp_mg_l, csed_mg_l, wn_kg, wp_kg,wsed_ton, wn_ret_kg, wp_ret_kg, wsed_ret_ton,study_case_id,case):
	listResult = []
	conn = connect('postgresql_alfa')
	cursor = conn.cursor()
	cursor.callproc('__wp_ptap_insert_report',[element_id,ptap_id,year,user_id,awy,cn_mg_l, cp_mg_l, csed_mg_l, wn_kg, wp_kg,wsed_ton, wn_ret_kg, wp_ret_kg, wsed_ret_ton,study_case_id,case])
	conn.commit()
	cursor.close()
	conn.close()
