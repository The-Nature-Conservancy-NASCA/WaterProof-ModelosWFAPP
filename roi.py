...
def DataCSVRoi(user_id, studycase, date):
    ...
    genCSVCost(studycase, '__wp_roi_cost', 'intake',
               'NBS', '1_CostFunction_NBS_Cap.csv')
    genCSVCost(studycase, '__wp_roi_cost', 'intake',
               'BAU', '2_CostFunction_BaU_Cap.csv')
    genCSVCost(studycase, '__wp_roi_cost', 'PTAP',
               'NBS', '3_CostFunction_NBS_PTAP.csv')
    genCSVCost(studycase, '__wp_roi_cost', 'PTAP',
               'BAU', '4_CostFunction_BaU_PTAP.csv')
    genCSVNBS_Cost(studycase, '__wp_roi_nbs_cost', '5_NBS_Cost.csv')
    genCSVPort_NBS(studycase, '__wp_dissagregation_nbs_first',
                   '__wp_dissagregation_nbs_second', '6_Porfolio_NBS.csv')
    genCSVFin_Par(studycase, '__wp_roi_financial_parameters_first',
                  '__wp_roi_financial_parameters_second', '7_Financial_Parmeters.csv')
...
def genCSVPort_NBS(studycase, function_id1, function_id2, csv_in):
    header = ["Time"]
...
def genCSVFin_Par(studycase, function_id1, function_id2, csv_in):
...
def genCSVCO2(studycase, csv_in, csv_out, date, user_id):
    path_principal = path.join(
        ruta, 'salidas', f'{user_id}_{studycase}_{date}')
    contenido = os.listdir(path_principal)
    df_carbon = []
    for direc in contenido:
        path_intake = path.join(path_principal, direc,
                                'out', '07-DISAGGREGATION', csv_in)
